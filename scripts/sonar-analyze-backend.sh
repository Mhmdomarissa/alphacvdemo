...
# SonarQube Analysis Script for Python Backend
# This script runs SonarQube analysis on the backend codebase

set -e

echo "🔍 Starting SonarQube analysis for Python Backend..."
...
cd "$(dirname "$0")/../alpha-backend" || exit 1
...
echo "📊 View results at: $SONAR_HOST_URL/dashboard?id=alpha-cv-backend"
