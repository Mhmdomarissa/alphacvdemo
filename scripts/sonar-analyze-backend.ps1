# SonarQube Analysis Script for Python Backend (PowerShell)
# This script runs SonarQube analysis on the backend codebase

$ErrorActionPreference = "Stop"

Write-Host "🔍 Starting SonarQube analysis for Python Backend..." -ForegroundColor Cyan
...
$backendDir = Join-Path $projectRoot "alpha-backend"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "📊 View results at: $sonarHostUrl/dashboard?id=alpha-cv-backend" -ForegroundColor Cyan
} else {
